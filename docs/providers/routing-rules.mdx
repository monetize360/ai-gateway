...
2. Set governance `semantic_routing.router.mode` to `plugin`
...
5. Plugin mode uses `router.mode=plugin`; HTTP sidecar mode uses `router.mode=http` plus `router.base_url`
