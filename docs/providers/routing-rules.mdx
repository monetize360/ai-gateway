...
- **Selection**: **Pin provider/model** (default) writes a specific target when CEL matches. **Semantic** picks a capable model from the intersection of the virtual key's allowlist and org/provider access policy (the same checks PreLLMHook enforces). Layer 1 extracts the request profile (modalities, tools, context size) and hard-filters the pool. When configured, Layer 2 calls vLLM-SR preview and intersects its recommendation with that filtered pool. Router failures, out-of-pool answers, or an unset router decline the rewrite (the incoming model is kept). The caller's requested provider/model does not bias selection when a rewrite commits. Semantic is not the default — a rule must set `action: "semantic"`, or `default_for_all` must be true.
...
**Semantic selection example** — CEL still decides when the rule fires; Layer 1 filters the virtual key pool, then vLLM-SR preview picks a model from that pool:
...
When `router.base_url` is configured, Layer 2 calls the vLLM-SR routing preview API synchronously.
The request contains the configured entrypoint and OpenAI-style messages. The preview endpoint does
not accept `eligible_models`, so Bifrost intersects `selected_model` and `recommended_models` with
the Layer 1 capability-filtered pool before committing. Unknown or out-of-pool model IDs are dropped.
If the router times out, returns an error, leaves no eligible recommendation, or is unset,
Bifrost declines the rewrite and keeps the incoming model. There is no local keyword/lexical
Layer 2. The router's `timeout_ms` applies to the HTTP call separately from the 10ms local-work
budget. The deprecated `classifier` config block is ignored.
...
truncated at 8000 characters, which caps what the router's `context` signals can observe — Layer 1
remains authoritative for whether a model's context window actually fits the request.
...
Layer 1 pool, Bifrost uses the next recommendation from the same decision and logs the substitution
at `2/route`.

Prefer **inline `model_overrides`** (or catalog DB fields) in production. `model_overrides_file` is
for local/PoC catalogs and should use a path relative to the process working directory / `-app-dir`.
Decision model IDs in the sidecar must match Bifrost `provider/model` allowlist strings.

### Local vs production runbooks

**Local validation** (loopback management API on `:18080`):

1. Start Docker Desktop, then `cd plugins/governance/poc/vllm_sr && ./start.sh`
2. `curl -sS http://127.0.0.1:18080/ready`
3. Merge [examples/governance/semantic-routing.local.json](https://github.com/maximhq/bifrost/blob/main/examples/governance/semantic-routing.local.json) into the governance plugin config (`router.base_url=http://127.0.0.1:18080`)
4. Hit Bifrost with a VK; logs should show `2/route: vllm-sr selected=...`
5. Optional sidecar-only debug: Postman collection in `plugins/governance/poc/vllm_sr/postman.json`

See also `plugins/governance/poc/README.md`.

**Production**:

1. Deploy the vLLM-SR management API; use the checklist in `plugins/governance/poc/vllm_sr/PROD-CHECKLIST.md`
2. Set `router.base_url` to the internal service URL; inject `router.api_key` via secrets
3. Prefer `action: "semantic"` routing rules (`default_for_all: false`) and inline `model_overrides`
4. Keep `timeout_ms` sized for your SLO; monitor decline / circuit-open rates in logs (`2/route: vllm-sr failed`, `declining semantic rewrite`, `circuit open`)
5. Sample shape: [examples/governance/semantic-routing.prod.json](https://github.com/maximhq/bifrost/blob/main/examples/governance/semantic-routing.prod.json)

`classes` / `default_class` are optional preference lists used when ranking L1 survivors
...
**Omitted `model`** — Additional trigger only. If the request has a virtual key, semantic routing is enabled, no pin matched, and the body has no `model` (OpenAI-style JSON; Gemini/Bedrock path models still count as present), the same Layer 1 filter + Layer 2 router/ranker runs even when `default_for_all` is false. Pin, CEL `action: "semantic"`, and `default_for_all` are unchanged. If selection cannot commit, the handler still returns `model is required`.
