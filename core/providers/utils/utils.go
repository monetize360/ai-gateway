...
// TruncateForLog returns s truncated to maxLen runes with an ellipsis when longer.
...
// RawResponseToString flattens ExtraFields.RawResponse into a compact string for logs/messages.
func RawResponseToString(raw interface{}) string {
	if raw == nil {
		return ""
	}
	switch v := raw.(type) {
...
// ExtractAlternateProviderErrorMessage tries common non-BifrostError JSON shapes
// (detail, message, error as string, nested error.message) when structured parse left Message empty.
func ExtractAlternateProviderErrorMessage(raw interface{}) string {
	body := RawResponseToString(raw)
	if body == "" {
		return ""
	}
	if !gjson.Valid(body) {
...
// LogProviderAPIError emits a Warn-level log for upstream provider HTTP errors so they
// are visible at default LOG_LEVEL=info (provider failure paths previously only logged at Debug).
func LogProviderAPIError(resp *fasthttp.Response, bifrostErr *schemas.BifrostError) {
	if bifrostErr == nil {
		return
	}
	status := 0
	...
	bodySnippet := TruncateForLog(RawResponseToString(bifrostErr.ExtraFields.RawResponse), 512)
	getLogger().Warn(
		"provider API error: status=%d content_type=%q message=%q body=%q provider=%q model=%q request_type=%q",
		status,
		contentType,
		msg,
		bodySnippet,
