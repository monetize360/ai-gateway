...
export function RoutingRuleSheet({ open, onOpenChange, editingRule, onSuccess }: RoutingRuleDialogProps) {
	...
	useEffect(() => {
		if (editingRule) {
			...
			setValue("action", editingRule.action === "semantic" ? "semantic" : "pin");
	...
	const onSubmit = (data: RoutingRuleFormData) => {
		...
		if (data.key_id?.trim() && !data.provider?.trim() && data.action !== "semantic") {
			toast.error("Provider is required when pinning a key");
			return;
		}

		if (data.action === "semantic" && data.chain_rule) {
			toast.error("Chain rule is not supported with semantic selection");
			return;
		}
		...
		const isSemantic = data.action === "semantic";
		const payload = {
			name: data.name,
			description: data.description,
			cel_expression: data.cel_expression,
			action: data.action,
			provider: isSemantic ? "" : data.provider?.trim() || undefined,
			model: isSemantic ? "" : data.model?.trim() || undefined,
			key_id: isSemantic ? "" : data.key_id?.trim() || undefined,
			fallbacks: validFallbacks,
			scope: data.scope,
			scope_id: data.scope === "global" ? undefined : data.scope_id || undefined,
			priority: data.priority,
			enabled: data.enabled,
			chain_rule: isSemantic ? false : data.chain_rule,
			query: query,
		};
					...
					<div className="flex grow flex-col gap-6 px-8 pb-6">
						...
						<div className="flex items-center justify-between rounded-lg border p-4">
							...
							<Switch
								...
								disabled={action === "semantic"}
								data-testid="routing-rule-chain-rule-switch"
							/>
						</div>

						{/* Selection: pin vs semantic */}
						<div className="space-y-3">
							<Label>Selection</Label>
							<Select
								value={action}
								onValueChange={(value) => {
									const next = value === "semantic" ? "semantic" : "pin";
									setValue("action", next);
									if (next === "semantic") {
										setValue("chain_rule", false);
										setValue("provider", "");
										setValue("model", "");
										setValue("key_id", "");
									}
							...
							>
								<SelectTrigger className="w-full" data-testid="routing-rule-action-select">
									<SelectValue placeholder="Select how this rule routes..." />
								</SelectTrigger>
								<SelectContent>
									<SelectItem value="pin">Pin provider / model</SelectItem>
									<SelectItem value="semantic">Semantic (pick from virtual key pool)</SelectItem>
								</SelectContent>
							</Select>
							<p className="text-muted-foreground text-xs">
								{action === "semantic"
									? "When this rule matches, Bifrost picks a capable model from the request's virtual key pool using class preference lists, model overrides, and the optional vLLM-SR router from the governance plugin config."
									: "When this rule matches, the request is sent to the provider and model below. Leave them empty to keep the incoming values."}
							</p>
						</div>
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<div>
									<Label>{action === "semantic" ? "Preferred models" : "Fallbacks"}</Label>{" "}
									<p className="text-muted-foreground mt-0.5 text-xs">
										{action === "semantic"
											? "Optional ordered preference list for this rule, best first. Overrides the plugin class preference list. Provider is required."
							...
							<p className="text-muted-foreground text-xs">
								{action === "semantic"
									? "Preferred models are tried in the order they are defined"
