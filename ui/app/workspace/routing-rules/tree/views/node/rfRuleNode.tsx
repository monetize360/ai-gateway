...
export function RFRuleNode({ data }: { data: any }) {
	...
	const isSemantic = rule.action === "semantic";
	const isPassthrough = !isSemantic && !rule.provider && !rule.model;
	...
	return (
		...
		>
			...
			>
				...
				>
					{rule.provider ? (
						<RenderProviderIcon provider={rule.provider as ProviderIconType} size={12} />
					) : (
					...
					<span className="text-muted-foreground truncate text-[10px]">
						{isSemantic
							...
							: isPassthrough
								? "Passthrough"
								: rule.model
									? `${getProviderLabel(rule.provider!)} / ${rule.model}`
									: getProviderLabel(rule.provider!)}
					</span>
				</div>
				...
				{hovered && (
					...
					>
						...
						<p className="mb-1 px-3 text-[10px] font-semibold tracking-wide uppercase" style={{ color: scopeColor }}>
							{rule.chain_rule ? "Resolved output (new input)" : "Routing Output"}
						</p>
						<div className="hover:bg-muted flex items-center gap-2 px-3 py-1.5">
							{rule.provider ? (
								<RenderProviderIcon provider={rule.provider as ProviderIconType} size={13} />
							) : (
								<span className="bg-muted-foreground/30 h-3 w-3 flex-shrink-0 rounded-full" />
							)}
							<div className="min-w-0 flex-1">
								<p className="text-foreground truncate text-xs font-medium">
									{isSemantic ? "Semantic" : isPassthrough ? "Passthrough" : rule.provider ? getProviderLabel(rule.provider) : rule.model}
								</p>
								{!isSemantic && rule.model && rule.provider && <p className="text-muted-foreground truncate font-mono text-[10px]">{rule.model}</p>}
								{isSemantic && (
									<p className="text-muted-foreground/60 text-[10px] italic">pick from virtual key pool</p>
								)}
