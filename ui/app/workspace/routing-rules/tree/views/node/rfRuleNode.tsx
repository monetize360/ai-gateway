...
export function RFRuleNode({ data }: { data: any }) {
	...
	const isPassthrough = !rule.provider && !rule.model;
			...
			>
				...
				>
					...
					<span className="text-muted-foreground truncate text-[10px]">
						{isPassthrough ? "Passthrough" : rule.model ? `${getProviderLabel(rule.provider!)} / ${rule.model}` : getProviderLabel(rule.provider!)}
					</span>
				</div>
						...
						<div className="hover:bg-muted flex items-center gap-2 px-3 py-1.5">
							...
							<div className="min-w-0 flex-1">
								<p className="text-foreground truncate text-xs font-medium">
									{isPassthrough ? "Passthrough" : rule.provider ? getProviderLabel(rule.provider) : rule.model}
								</p>
								{rule.model && rule.provider && <p className="text-muted-foreground truncate font-mono text-[10px]">{rule.model}</p>}
