...
func TestApplyGovernanceRefreshDelta_BudgetSoftLimitSurvivesVirtualKeyRefresh(t *testing.T) {
	logger := NewMockLogger()
	budget := buildBudgetWithUsage("budget1", 10.0, 15.0, "1d")
	...
	vk.ProviderConfigs = []configstoreTables.TableVirtualKeyProviderConfig{{
		ID:       "pc1",
		Provider: "openai",
		Budgets:  []configstoreTables.TableBudget{{ID: budget.ID, MaxLimit: 10.0, SoftLimit: false, ResetDuration: "1d"}},
	}}
	...
	updatedBudget.SoftLimit = true
	delta := &configstore.GovernanceRefreshDelta{
		VirtualKeys: []configstoreTables.TableVirtualKey{staleVK},
		Budgets:     []configstoreTables.TableBudget{updatedBudget},
	}
	store.applyGovernanceRefreshDelta(context.Background(), delta)

	loaded := store.LoadBudget(context.Background(), budget.ID)
	require.NotNil(t, loaded)
	assert.True(t, loaded.SoftLimit, "authoritative budget delta must win over stale embedded VK budgets")

	decision, err := store.CheckBudget(context.Background(), EntityWiseBudgets{
		"VK": {loaded},
	}, nil)
	require.Error(t, err)
	assert.Equal(t, DecisionBudgetExceededSoft, decision)
}

func TestApplyGovernanceRefreshDelta_HardBudgetEnforcedAfterSoftLimitDisabled(t *testing.T) {
	logger := NewMockLogger()
	budget := buildBudgetWithUsage("budget1", 10.0, 15.0, "1d")
	...
	vk.ProviderConfigs = []configstoreTables.TableVirtualKeyProviderConfig{{
		ID:       "pc1",
		Provider: "openai",
		Budgets:  []configstoreTables.TableBudget{{ID: budget.ID, MaxLimit: 10.0, SoftLimit: true, ResetDuration: "1d"}},
	}}
	...
	loaded := store.LoadBudget(context.Background(), budget.ID)
	require.NotNil(t, loaded)
	assert.False(t, loaded.SoftLimit, "authoritative budget delta must disable soft limit")
...
func TestUpdateVirtualKeyInMemory_DoesNotClobberCanonicalSoftLimit(t *testing.T) {
	logger := NewMockLogger()
	budget := buildBudgetWithUsage("budget1", 10.0, 15.0, "1d")
	budget.SoftLimit = false
	...
	staleVK := *vk
	staleVK.Budgets[0].SoftLimit = true
	staleVK.ProviderConfigs = []configstoreTables.TableVirtualKeyProviderConfig{{
		ID:       "pc1",
		Provider: "openai",
		Budgets:  []configstoreTables.TableBudget{{ID: budget.ID, MaxLimit: 10.0, SoftLimit: true, ResetDuration: "1d"}},
	}}

	store.UpdateVirtualKeyInMemory(context.Background(), &staleVK, nil, nil, nil)

	loaded := store.LoadBudget(context.Background(), budget.ID)
	require.NotNil(t, loaded)
	assert.False(t, loaded.SoftLimit, "VK association reload must not revert canonical soft_limit=false")
