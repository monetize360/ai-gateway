...
func (r *OpenAIRouter) SelectModelForEval(
	...
	if len(decision.ModelRefs) == 0 && decision.CapabilityProfile != nil {
		return services.EvalModelSelection{
			Status: services.EvalSelectionProfileOnly,
			Method: "capability_profile",
			Reason: "the decision declares required model qualities; inventory selection is the caller's",
		}
	}
