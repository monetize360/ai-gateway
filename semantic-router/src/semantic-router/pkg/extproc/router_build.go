...
func NewOpenAIRouter(configPath string) (*OpenAIRouter, error) {
	...
	router, err := NewOpenAIRouterFromConfig(cfg)
	if err != nil {
		return nil, err
	}
...
// NewOpenAIRouterFromConfig builds the routing runtime from an already parsed
// canonical configuration without publishing package-global configuration or
// starting an HTTP/Envoy server. Embedded consumers use this constructor to
// share the production signal, decision, and model-selection implementation.
func NewOpenAIRouterFromConfig(cfg *config.RouterConfig) (*OpenAIRouter, error) {
	return buildOpenAIRouterFromConfig(cfg)
}
